...
class TwoLayerNet(object):
    ...
    def loss(self, X, y=None):
        ...
        a2_out, a2_cache = affine_forward(a1_out, self.params['W2'], self.params['b2'])
        scores = a2_out
...
class FullyConnectedNet(object):
    ...
    def loss(self, X, y=None):
        ...
        for lay in range(self.num_layers-1):
            if self.normalization:
                layer_input, a_cache[lay] = affine_bn_relu_forward(layer_input, self.params['W%d'%(lay+1)], self.params['b%d'%(lay+1)], self.params['gamma%d'%(lay+1)], self.params['beta%d'%(lay+1)], self.bn_params[lay])
            else:
                layer_input, a_cache[lay] = affine_relu_forward(layer_input, self.params['W%d'%(lay+1)], self.params['b%d'%(lay+1)])
            
            if self.use_dropout:
                layer_input, dp_cache[lay] = dropout_forward(layer_input, self.dropout_param)
         
        a_out, a_cache[self.num_layers] = affine_forward(layer_input, self.params['W%d'%(self.num_layers)], self.params['b%d'%(self.num_layers)])
        scores = a_out
        ...
        dx, dw, db = affine_backward(dout, a_cache[self.num_layers])
        ...
        for idx in range(self.num_layers - 1):
            ...
            if self.normalization:
                dx, dw, db, dgamma, dbeta = affine_bn_relu_backward(dout, a_cache[lay])
            else:
                dx, dw, db = affine_relu_backward(dout, a_cache[lay])
